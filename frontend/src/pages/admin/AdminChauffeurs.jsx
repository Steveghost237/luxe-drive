...
import { Search, UserCheck, UserX, Loader2, RefreshCw, Trash2, Shield } from 'lucide-react'
import { useAdminUtilisateurs } from '../../hooks/useAdminData'

export default function AdminChauffeurs() {
  const [search, setSearch] = useState('')
  const [actionId, setActionId] = useState(null)
  const { data: chauffeurs, loading, refresh } = useAdminUtilisateurs('chauffeur', search)

  const actifs   = chauffeurs.filter(c => c.est_actif).length
  const inactifs = chauffeurs.filter(c => !c.est_actif).length
  const verifies = chauffeurs.filter(c => c.est_verifie).length
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-display font-bold text-white">Chauffeurs</h1>
          <p className="text-gray-500 text-sm mt-0.5">{loading ? '…' : `${chauffeurs.length} chauffeurs dans la base de données`}</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label:'Total',     value: loading ? '…' : chauffeurs.length, color:'text-white'     },
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-gray-800">
              <tr>{['Chauffeur','Téléphone','Email','Vérifié','Statut','Inscrit le','Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-800/50">
              ...
              {!loading && chauffeurs.length === 0 && (
                <tr><td colSpan={7} className="py-12 text-center text-gray-600 text-sm">Aucun chauffeur trouvé.</td></tr>
              )}
              {!loading && chauffeurs.map(c => {
                const busy = actionId === c.id
                return (
                  <tr key={c.id} className="hover:bg-white/2 transition-colors">
                    <td className="py-3.5 px-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gold-400/30 to-gold-600/30 flex items-center justify-center text-gold-400 text-xs font-bold shrink-0">
