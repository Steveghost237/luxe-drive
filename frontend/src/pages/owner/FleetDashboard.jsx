...
import {
  Car, TrendingUp, Satellite, Wrench,
  LogOut, LayoutDashboard, Settings, Bell
} from 'lucide-react'
import useAuthStore from '../../store/authStore'
import ChatBubble from '../../components/ChatBubble'
import FleetHome          from './FleetHome'
import FleetVehicules     from './FleetVehicules'
import FleetRevenus       from './FleetRevenus'
import FleetGPS           from './FleetGPS'
...
const NAV = [
  ...
  { to: 'vehicules',             icon: Car,              label: 'Ma flotte'       },
  { to: 'revenus',               icon: TrendingUp,       label: 'Revenus'         },
  { to: 'gps',                   icon: Satellite,        label: 'Suivi GPS'       },
  { to: 'maintenance',           icon: Wrench,           label: 'Maintenance'     },
  { to: 'notifs',                icon: Bell,             label: 'Notifications', badge: 3 },
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-60 bg-gray-900/95 border-r border-gray-800 flex flex-col z-40">
      <div className="p-6 border-b border-gray-800">
        ...
        <p className="text-gray-600 text-xs mt-1.5 uppercase tracking-widest">Espace Propriétaire</p>
      </div>
      ...
      <div className="px-4 py-4 border-b border-gray-800/60">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gold-400 to-gold-600 flex items-center justify-center shrink-0">
            <span className="text-black text-sm font-bold">{(user?.prenom?.[0] || 'P').toUpperCase()}</span>
          </div>
          <div className="min-w-0">
            <p className="text-white text-sm font-medium truncate">{user?.prenom} {user?.nom}</p>
            <p className="text-gray-500 text-xs capitalize">Propriétaire</p>
          </div>
        </div>
      </div>
          ...
          <NavLink key={to} to={to} end={end}
            className={({ isActive }) =>
              `w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                isActive ? 'bg-gold-500/10 text-gold-400 border border-gold-500/20' : 'text-gray-500 hover:text-white hover:bg-gray-800'
              }`
            }>
            <Icon size={16} className="shrink-0" />
            <span className="flex-1">{label}</span>
            {badge ? <span className="bg-gold-500 text-black text-xs font-bold px-1.5 py-0.5 rounded-full">{badge}</span> : null}
          </NavLink>
        ))}
      </nav>
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
      ...
      <main className="ml-60 flex-1 overflow-auto">
        <Routes>
          <Route index                element={<FleetHome />} />
          <Route path="vehicules"     element={<FleetVehicules />} />
          <Route path="revenus"       element={<FleetRevenus />} />
          <Route path="gps"           element={<FleetGPS />} />
          <Route path="maintenance"   element={<FleetMaintenance />} />
          <Route path="notifs"        element={<FleetNotifications />} />
